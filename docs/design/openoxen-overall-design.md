...
- `src/openoxen`
  - OpenOxen 本地状态路径与目录布局（`~/.openoxen`）的统一定义。
...
- `openoxen skills install` 支持按 GitHub URL 或技能名安装技能。
...
- 默认状态根目录：`~/.openoxen`（可由 `OPENOXEN_HOME` 覆盖）。
- auth/config/memory/skills/logs/caches 统一归档在该目录下。
- pipeline 日志写入 `~/.openoxen/logs/<project>-<hash>/pipeline.<timestamp>/`。
